...
export function decode(dna:any, propertySizes:any){
    const results:any[] = [];
...
export function encode(propertySizes:any, propertyValues:any){
    let i = 0;
    let dna = 0;
    let factor = 1;
    while(i < propertySizes.length){        
...
export function getMetadataFromDna(dna){
...
export function getMetadataFromIndexes({eyeIndex, mouthIndex, eyeColorIndex, headIndex}){
